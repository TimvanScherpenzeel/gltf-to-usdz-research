...
function constructOBJ(mesh) {
  ...
  function packToUInt16Array(data) {
    ...
    for (let i = 0; i < uintArray.length; i += 1) {
      [uintArray[i]] = new Uint16Array(
        new Uint8Array([data[byteOffset], data[byteOffset + 1]]).buffer,
      );
      byteOffset += 2;
    }
    return uintArray;
  ...
  if (verticesData) {
    ...
    // Order and sign is normally wrong (due to different coordinate system?) (x, y, z)
    // v 0.646051287651062 -0.6916618943214417 -0.047537729144096375

    // Which should be (x, z, y)
    ...
    for (let i = 0; i < verticesFloatArray.length; i += 3) {
      ...
      // I originally thought this would be verticesMax[2] / 2 as you would
      // move from the center to the bottom but when testing it shows lower
      // than the reference apple.obj model (which was confirmed to be correct)
  ...
  const objFullpath = `${outputFilePath}/${getFilename(inputFileName)}.obj`;
  try {
    fs.writeFileSync(objFullpath, objData);
  } catch (err) {
    return Promise.reject(err);
  }

  console.log('Written file to', objFullpath);
  return Promise.resolve({
    objPath: `${getFilename(inputFileName)}.obj`,
  });
}
