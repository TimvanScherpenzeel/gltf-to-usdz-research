function constructUSDA(data) {
	...
	def Scope "Materials"
	{
		...
		def Material "StingrayPBS_0"
		{
			...
			${colorTexturePath ? createMapSection('color_map', colorTexturePath) : ''}

			${emissiveTexturePath ? createMapSection('emissive_map', emissiveTexturePath) : ''}

			${metallicTexturePath ? createMapSection('metallic_map', metallicTexturePath) : ''}

			${normalTexturePath ? createMapSection('normal_map', normalTexturePath) : ''}

			${aoTexturePath ? createMapSection('ao_map', aoTexturePath) : ''}

			${roughnessTexturePath ? createMapSection('roughness_map', roughnessTexturePath) : ''}
		}
